...
class Adv_model extends CI_Model {
     ...
     * 广告位列表
     */
    public function queryAdPositionList ($per_page=10) {
    	$onset = $this->uri->segment(5)==''?0:$this->uri->segment(5);
		$sql = "select * from {$this->db->dbprefix}adv where flag=1 ";
		$totalSql = "select count(*) as counts from(".$sql.") as a";
		$query = $this->db->query($totalSql);
		$total = $query->row_array();
		$sql .= " order by adv_id desc limit {$onset}, {$per_page}";
