	...
	<div class="inner-body">
		<div class="top-info">
			...
			<div class="rant clearfix">
				<p>1<i class="fa fa-diamond"></i>=<?php echo 1*$bdrant?>&nbsp;<i class="fa fa-btc"></i></p>	
			</div>
		</div>
		<div class="main-info">
			<input id="bdrant" type="hidden" value="<?php echo $bdrant?>">
			<div class="info-group clearfix">
				<label class="group-left" for="">所需B币数数：</label>
				<div class="group-right">
					<input type="hidden" >
					<p><span id="change-need">00.00</span>&nbsp;<i class="fa fa-btc"></i></p>
				</div>
			</div>
			<div class="info-group clearfix">
				<label class="group-left" for="">兑换钻石数：</label>
				<div class="group-right">
					<input type="text" id="diamond" class="ipt"><em>钻石</em>
				</div>
			</div>
			<!--<div class="info-group clearfix">
...
<script type="text/javascript">
	$(function(){
		$('#diamond').on('input propertychange', function () {
			var diamond = $(this).val();
			var bdrant = $('#bdrant').val();
			var need = diamond*bdrant;
			$('#change-need').text(need.toFixed(2));
		})
		$('#changeBtn').on('click', function () {
			$.post("<?php echo site_url('Ajax/BIconToDiamond') ?>", {
				diamond : $('#diamond').val(),
			}, function(response){
				if (response.status==1){
					...
					layer.tips('输入的钻石必须大于零','#diamond',{
						tips : 2
					})
				}else if(response.status==-3){
					layer.tips('没有那么多B币','#change-need',{
						tips : 1
					})
				}else{
